...
# 1. Installs packages: ssh, sudo, openssl.
# 2. Setup the sshd daemon (Create in /etc/ssh/sshd_config.d/proxmox.conf and set port, setup security)
# 3. Setup the sudoers file (Create group, add to file group)
# 4. Creates a user with sudo group
# 5. Add to new user ssh authorized keys
#
...
echo " "
echo "Packages to install: ssh, sudo, openssl"
echo "Enter to continue"
read
echo " "
apt-get install -y ssh sudo openssl
